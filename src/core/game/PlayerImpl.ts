...
import { TileDonationImpl } from "./TileDonationImpl";
import {
...
export class PlayerImpl implements Player {
	...
	public pastOutgoingVassalageRequests: VassalageRequest[] = [];
	...
	private lastIncomingAttackTick = new Map<Player, number>();
	private lastOutgoingAttackTick = new Map<Player, number>();

	public _incomingTileDonations: TileDonation[] = [];
	public _outgoingTileDonations: TileDonation[] = [];
	...
	toUpdate(): PlayerUpdate {
		...
		const outgoingVassalageRequests = this.outgoingVassalageRequests().filter(vr => vr.empire() === this).map(vr => vr.recipient().id());
		const outgoingSelfVassalageRequests = this.outgoingVassalageRequests().filter(vr => vr.vassal() === this).map(vr => vr.recipient().id());
		...
		return {
			...
			outgoingVassalageRequests: outgoingVassalageRequests,
			outgoingSelfVassalageRequests: outgoingSelfVassalageRequests,
			vassalages: this.vassalages().map(
				(v) => ({
					id: v.id(),
					other: v.other(this).id(),
					isOtherVassal: v.empire() === this,
	...
	isTransitivelyFriendly(other: Player, treatAFKFriendly: boolean = false): boolean {
		let empire = other.empire();
		while(empire) {
			if(this.isFriendly(empire, treatAFKFriendly)) {
				return true;
			}
	...
	incomingVassalageRequests(): VassalageRequest[] {
		return this.mg.vassalageRequests.filter((vr) => vr.recipient() === this);
	}

	outgoingVassalageRequests(): VassalageRequest[] {
		return this.mg.vassalageRequests.filter((vr) => vr.requestor() === this);
	}

	vassalages(): Vassalage[] {
		return this.mg.vassalages_.filter(
			(v) => v.requestor() === this || v.recipient() === this
		);
	}
	
	empire(): Player | null {
		return this.vassalages().find((v) => v.vassal() === this)?.empire() ?? null;
	}
	
  	vassals(): Player[] {
		return this.vassalages().filter((v) => v.empire() === this).map(v => v.vassal());
	}
	
	isVassalOf(other: Player): boolean {
		return this.empire() === other;
	}
	
  	isEmpireOf(other: Player): boolean {
		return other.empire() === this;
	}
	
	removeAllVassalages(): void {
		this.mg.removeVassalagesByPlayerSilently(this);
	}
  	...
  	vassalageWith(other: Player): Vassalage | null {
		...
		return (
			this.vassalages().find(
				(v) => v.recipient() === other || v.requestor() === other,
			) ?? null
		);
	}
	
	hasVassalageWith(other: Player): boolean {
		return this.vassalageWith(other) !== null;
	}
	
	canBecomeVassalOf(other: Player): boolean {
		return other.canBecomeEmpireOf(this);
	}
	...
	canBecomeEmpireOf(other: Player): boolean {
		...
		if (this.hasVassalageWith(other) || !this.isAlive() || !other.isAlive()) {
			return false;
		}
  	...
  	canSendVassalageRequest(other: Player): boolean {
		if(!this.canBecomeEmpireOf(other)) {
			return false;
		}

		const hasPending = this.outgoingVassalageRequests().some(
			(vr) => vr.recipient() === other,
		);
		...
		const incoming = this.incomingVassalageRequests().find(
			(vr) => vr.requestor() === other,
		);

		if (incoming && incoming.vassal() === other) {
			return true;
		}
		
		const ticks = this.mg.ticks();
		const cooldown = this.mg.config().allianceRequestCooldown()
		
		const recent = this.pastOutgoingVassalageRequests
			.filter((vr) => vr.recipient() === other && ticks - vr.createdAt() < cooldown)

		return recent.length === 0;
	}
	
  	canSendSelfVassalageRequest(other: Player): boolean {
		if(!this.canBecomeVassalOf(other)) {
			return false;
		}
		...
		if (incoming && incoming.empire() === other) {
			return true;
		}
  	...
  	breakVassalage(vassalage: Vassalage): void {
		this.mg.breakVassalage(this, vassalage);
	}
	
  	createVassalageRequest(recipient: Player): VassalageRequest | null {
		if (this.hasVassalageWith(recipient)) {
			throw new Error(`cannot create vassalage request, already vassalage`);
		}
		return this.mg.createVassalageRequest(this, recipient, recipient, this);
	}
  	...
  	createSelfVassalageRequest(recipient: Player): VassalageRequest | null {
		...
		return this.mg.createVassalageRequest(this, recipient, this, recipient);
	}
	...
	updateRelation(other: Player, delta: number): void {
		...
		const relation = this.relations.get(other) ?? 0;
		const maxBound = this.hasVassalageWith(other) ? 200 : 100;
		const newRelation = within(relation + delta, -maxBound, maxBound);
	...
	canDonateTiles(recipient: Player): boolean {
		...
		if (
			recipient.type() === PlayerType.Human &&
			!this.mg.config().donateTiles()
		) {
			return false;
		}
		
		if(this.sentDonations.find(d => d.recipient === recipient
			&& this.mg.ticks() - d.tick < this.mg.config().donateCooldown())) {
			return false;
		}
		return this.sharesBorderWith(recipient);
	}
	...
	isFriendly(other: Player, treatAFKFriendly: boolean = false): boolean {
		...
		return this.isOnSameTeam(other) || this.isAlliedWith(other) || this.hasVassalageWith(other);
	}
	...
	weightedEmpireTroops(weight?: number): number {
		weight = weight ?? 0.7;
		return Number(this._troops) + ((this.empire()?.troops()) ?? 0) * weight;
	}
  ...
  warshipSpawn(tile: TileRef): TileRef | false {
	...
	createAttack(
		...
		if (target.isPlayer()) {
			...
			this.lastOutgoingAttackTick.set(target, this.mg.ticks());
			(target as PlayerImpl).lastIncomingAttackTick.set(this, this.mg.ticks());
		}
		return attack;
	}
	...
	getLastIncomingAttackTick(attacker: Player): number {
		return this.lastIncomingAttackTick.get(attacker) ?? -1;
	}
	
	lastIncomingAttackTicks(): [Player, number][] {
		return Array.from(this.lastIncomingAttackTick.entries());
	}
	
	getRecentAttackers(inPastTicks: number): Player[] {
		return this.lastIncomingAttackTicks().filter(entry => this.mg.ticks() - entry[1] <= inPastTicks).map(entry => entry[0]);
	}
	
	getLastOutgoingAttackTick(attacker: Player): number {
		return this.lastOutgoingAttackTick.get(attacker) ?? Number.MIN_SAFE_INTEGER;
	}
	
  	lastOutgoingAttackTicks(): [Player, number][] {
		return Array.from(this.lastOutgoingAttackTick.entries());
	}
	
  	getRecentAttackTargets(inPastTicks: number): Player[] {
		return this.lastOutgoingAttackTicks().filter(entry => this.mg.ticks() - entry[1] <= inPastTicks).map(entry => entry[0]);
	}
	...
	public canAttackPlayer(
		...
		switch (this.type()) {
			...
			case PlayerType.Nation:
				...
				return !player.isImmune() && !this.isFriendly(player, treatAFKFriendly) && !this.isTransitivelyFriendly(player, treatAFKFriendly);
			case PlayerType.Human:
				// Humans don't have to respect transitive friends
	...
	createTileDonation(
		...
		const tileDonation = new TileDonationImpl(
			this._pseudo_random.nextID(),
			recipient,
			this.mg.ticks(),
			this,
		...
		this._outgoingTileDonations.push(tileDonation);
		if (recipient.isPlayer()) {
			(recipient as PlayerImpl)._incomingTileDonations.push(tileDonation);
		}
		this.sentDonations.push(tileDonation);
		return tileDonation;
	}
	outgoingTileDonations(): TileDonation[] {
		return this._outgoingTileDonations;
	}
	incomingTileDonations(): TileDonation[] {
		return this._incomingTileDonations;
	}
}
